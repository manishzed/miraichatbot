...
import json
from BooksInfo.models import Books

from BooksInfo.models import Mir
...
class booklist(APIView):
    ...
    def get(self,request):
        ...
        if STATE =="WELCOME_ASK_NAME":
            row=Mir(ip=ip,userid=userid,currentstate="WELCOME_ASK_NAME",question='0', answer='0')
            row.save()
        #database=Mir.objects.raw('SELECT currentstate FROM mir order by id DESC')
        from django.db import connection
        cursor = connection.cursor()
        cursor.execute("SELECT  currentstate FROM mir order by id DESC")
        ...
        print(x)
        #print("ippp")
        #print(database[0].currentstate)
        
        #for p in database[0]:#
         #   print(p.ip)
            #print("%s is %s."%(p.currentstate, p.ip))
        print("hello00")
        #print(database.userid)
        print("hiii")
        #json_input = request.json
        inp_msg  = received_json_data['message']
        print(inp_msg)
        
        if STATE == "WELCOME_ASK_NAME":
            out_msg = welcome_ask_name()
            STATE = "NAME_ASKED"
            cur1=cursor.execute("SELECT  id FROM mir where ip=ip order by id DESC")
            k=cur1[0]
            sql="update  mir set currentstate = 'NAME_ASKED' where id=%s"
            param=[k]
        ...
        if STATE == "IS_NICKNAME":
            ...
            #books.objects.create(book_name="xyz",book_price=10,author_name="abc",book_quantity=10)
            p=Books(book_name=received_json_data["book_name"],book_price=received_json_data["book_price"],author_name=received_json_data["author_name"],book_quantity=received_json_data["book_quantity"])
            p.save()
            print("save")
